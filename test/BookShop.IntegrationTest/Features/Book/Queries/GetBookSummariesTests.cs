...
using BookShop.IntegrationTest.Features.Category.FakeData;
...
{
    public class GetBookSummariesTests : TestFeatureBase
    {
        public GetBookSummariesTests(WebAppFactoryFixture applicationCollectionFixture, ITestOutputHelper testOutputHelper)
        ...
        public async Task ShouldReturn()
        {
            ...
            await _TestRepository.Add<E.Book, Guid>(books);

            //Act
            PaginatedDtos<BookSummaryDto> paginatedBookSummaries = await _TestRequestHandler.SendRequest<GetBookSummariesQuery, PaginatedDtos<BookSummaryDto>>(new GetBookSummariesQuery
            {
        ...
        public async Task WithFilter_CategoryId_ShouldReturn_Filtered()
        {
            ...
            PaginatedDtos<BookSummaryDto> paginatedBookSummaries = await _TestRequestHandler.SendRequest<GetBookSummariesQuery, PaginatedDtos<BookSummaryDto>>(
        ...
        public async Task WithFilter_AuthorId_ShoudReturn_Filtered()
        {
            ...
            foreach (var book in books)
            {
                book.Author_Books = new List<E.Author_Book>();
                randoemCount = Random.Shared.Next(0, authors.Count);
                for (int i = 0; i <= randoemCount; i++)
                {
                    book.Author_Books.Add(new E.Author_Book
                    {
                        Author = authors[i],
                    });
                }
        ...
        public async Task WithFilter_StartPrice_ShouldReturn_Filtered()
        {
            ...
            //Act
            PaginatedDtos<BookSummaryDto> paginatedDtos = await _TestRequestHandler.SendRequest<GetBookSummariesQuery, PaginatedDtos<BookSummaryDto>>(new GetBookSummariesQuery
            {
        ...
        public async Task FromCache_ShouldReturn()
        {
            ...
            await _TestRequestHandler.SendRequest<GetBookSummariesQuery, PaginatedDtos<BookSummaryDto>>(request);

            //Act
            PaginatedDtos<BookSummaryDto>? paginatedBookSummaries = await _TestCache.GetFromCache<GetBookSummariesQuery, PaginatedDtos<BookSummaryDto>>(request);
