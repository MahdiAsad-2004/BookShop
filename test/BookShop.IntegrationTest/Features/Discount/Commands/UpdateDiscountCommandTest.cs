...
{
    public class UpdateDiscountCommandTest : TestFeatureBase
    {
        ...
        public UpdateDiscountCommandTest(WebAppFactoryFixture applicationCollectionFixture, ITestOutputHelper testOutputHelper)
        ...
        {
            await _TestRepository.Add<E.Discount, Guid>(_savedDiscount);
        }
        ...
        {
            //Arrnage 
            await _TestRepository.AddPermissionForUser(PermissionConstants.Discount.Update);

            //Act
            var result = await _TestRequestHandler.SendRequest<UpdateDiscountCommand, Result<Empty>>(updateDiscountCommand);

            //Assert
            Assert.NotNull(result);
            Assert.True(result.IsSuccess);
            E.Discount? entity = await _TestRepository.Get<E.Discount, Guid>(updateDiscountCommand.Id);
        ...
        public async Task Without_RequiredPermissions_ShouldThrow_UnauthorizeException()
        {
            ...
            //Act
            var task = _TestRequestHandler.SendRequest<UpdateDiscountCommand, Result<Empty>>(updateDiscountCommand);
